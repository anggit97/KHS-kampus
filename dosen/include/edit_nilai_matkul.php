		...
		<?php
			...
			if ($qeury_m) {
				if (mysqli_num_rows($qeury_m)>0) {
					...
					$query_mhs = mysqli_query($conn,"SELECT nama_mhs FROM mahasiswa WHERE nim='$nim'");
					if ($query_mhs) {
			...
			$query_nilai = mysqli_query($conn,"SELECT * FROM detil_ujian WHERE nim='$nim' AND id_ujian='$id_ujian'");
      if ($query_nilai) {
        $rows_n = mysqli_fetch_array($query_nilai);
      }else echo mysqli_error($conn);
    ...
    <a href="index.php?page=7&&kode_matkul=<?php echo $rows['kode_matkul']; ?>" class="btn btn-lg btn-primary" style="margin-right:10px;float:right;"><i class="fa fa-arrow-circle-left"></i> Kembali</a>
		...
		<div class="box box-info">
                ...
                <div class="box-body">
                  <form action="" method="post">
                    <div class="form-group">
                      ...
                      <input type="number" class="form-control" name="nil_absen" id="nil_absen" max="100" min="0" value="<?php echo $rows_n['nilai_absen']; ?>" placeholder="Nilai Absen">
                    </div>
                    <div class="form-group">
                      <label for="">Nilai Tugas : </label>
                      <input type="number" class="form-control" name="nil_tugas" id="nil_tugas" max="100" min="0" value="<?php echo $rows_n['nilai_tugas']; ?>" placeholder="Nilai Tugas">
                    </div>
                    <div class="form-group">
                      <label for="">Nilai UTS : </label>
                      <input type="number" class="form-control" name="nil_uts" id="nil_uts" max="100" min="0" value="<?php echo $rows_n['nilai_uts']; ?>" placeholder="Nilai UTS">
                    </div>
                    <div class="form-group">
                      <label for="">Nilai UAS : </label>
                      <input type="number" class="form-control" name="nil_uas" id="nil_uas" max="100" min="0" value="<?php echo $rows_n['nilai_uas']; ?>" placeholder="Nilai UAS">
                    </div>
                </div>
              ...
              if (isset($_POST['simpan'])) {
      					...
      					$query_nilai = mysqli_query($conn,"UPDATE detil_ujian set nilai_absen=$nil_absen,nilai_tugas=$nil_tugas,nilai_uts=$nil_uts,nilai_uas=$nil_uas WHERE id_ujian='$id_ujian' AND nim='$nim'");
      					if ($query_nilai) {
      						$_SESSION['pesan'] = "<div class='alert alert-success' style='margin-top:5px;'>
                                      <a href='#'' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                                      Berhasil Ubah Nilai Mahasiswa!
                                    </div>";
            					echo "<script>window.location = 'index.php?page=8&&id_ujian=$id_ujian&&nim=$nim'</script>";
      					}else{
      						$_SESSION['pesan'] = "<div class='alert alert-success' style='margin-top:5px;'>
                                      <a href='#'' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
