...
<?php 
  ...
  if (isset($_POST['simpan'])) {
    ...
    if ($nid == "" || $nama == "" || $jk == "" || $alamat == "" || $pass == "" || $con_pass == "" || $email == "") {
      ...
      $query_c = mysqli_query($conn,"SELECT * FROM dosen");
      $flag = 0;
      if ($query_c) {
      ...
      if ($flag == 1) {
        $_SESSION['pesan'] = "<div class='alert alert-danger' style='margin-top:5px;'>
                                <a href='#'' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                                NID yang diinput sudah digunakan oleh Dosen lain, Gunakan NID lain!
                              </div>";
        echo "<script>window.location = 'index.php?page=16'</script>";
      }else{
        $cost = 10;
        $hash = password_hash($pass, PASSWORD_BCRYPT, ["cost" => $cost]);
        $query = mysqli_query($conn,"INSERT INTO dosen (nid,nama_dosen,jk,alamat_dosen,password,email)
          VALUES('$nid','$nama','$jk','$alamat','$hash','$email')");
        if ($query) {
